...
Parameters and functions used to describe the tree are described as follows:"""
    ...
    def __init__(self, function_set=("add", "mul", "sub", "div", "cos", "sin"), \
        ...
        ##### Parameters added to make tree implementation uniform to that of rules (Not used) #####
        self.condition = None
        self.errorSum = 0
        self.errorCount = 0
        ...
        self.phenotype_RP = None  # Probability of this phenotype occurring by chance.
        self.accuracy = 0.0  # For classification: Accuracy calculated using only instances in the data set which the
        # tree matched. For regression: Sum of squares of the difference between true and predicted phenotype.
        self.accuracyComponent = None  # For classification Accuracy adjusted based on accuracy by random chance, and
        # transformed to give more weight to early changes in accuracy. For regression: Reciprocal of the RMSE.
    ...
    # Returns the string representation of the root which builds the representation of the whole tree recursively.
    def __str__(self):
    ...
    def _add_features_in_terminal_set(self, prefix):
        ...
        random.shuffle(self.terminal_set)
...
# For discrete phenotype
    def updateIndFitness(self, exploreIter):
        ...
        if self.coverDiff > 0:
            ...
            # -----------------------------------------------------------------------------------
            # Coverage is directly comparable when epoch complete, otherwise we want to estimate what coverage might be farther out.
            if self.epochComplete:
                # Get Pareto Metric
...
#####################################################################################
#        Population Initialization - Used as standalone GP population          #
#####################################################################################
...
#####################################################################################
#               Parent selection methods for applying genetic operators             #
#####################################################################################
...
if __name__ == "__main__":
    ...
    print("Tree representation:\n" + str(tree))  # Printing the tree. The tree is printed in the form of the actual tree. It starts with the root,
